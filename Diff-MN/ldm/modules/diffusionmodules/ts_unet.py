...
class UNetModel(nn.Module):
    ...
    def _forward(self, x, timesteps=None, context=None, mask=None, y=None, cond_drop_prob=0, **kwargs):
        ...
        :return: an [N x C x ...] Tensor of outputs.
        """
        # print(x.shape) #torch.Size([2048, 1, 28])
        # time.sleep(5400)
        # context = None
        ...
        for module in self.output_blocks:
            # print(h.shape,hs.pop().shape)
            skip_h=hs.pop()
            skip_h_last_dim=skip_h.shape[-1]
            h_last_dim=h.shape[-1]
            if skip_h_last_dim != h_last_dim:
                h = F.pad(h, (0, skip_h_last_dim-h_last_dim), mode='constant', value=0)
