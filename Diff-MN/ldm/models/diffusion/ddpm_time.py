...
class DDPM(pl.LightningModule):
    ...
    def training_step(self, batch, batch_idx):
        ...
        # print(loss_dict)
        self.loss_train_all.append(loss_dict['train/loss'].item())
        # print('****All Training Loss',self.loss_train_all)
    ...
    def validation_step(self, batch, batch_idx):
        ...
        self.loss_val_all.append(loss_dict_no_ema['val/loss_simple'].item())
        # print('****All Validation Loss',self.loss_val_all)
...
class LatentDiffusion(DDPM):
    ...
    def get_first_stage_encoding(self, encoder_posterior):
        ...
        # print(self.scale_factor) #1.0
        # print(z.shape) #torch.Size([128, 1, 168])
        return self.scale_factor * z

    def get_learned_conditioning(self, c, return_mask=False):
        # print(c.shape)#torch.Size([32, 24, 51])
    ...
    def get_input(self, batch, k, return_first_stage_outputs=False, force_c_encode=False,
        ...
        # print(batch['context'].shape) #torch.Size([128, 168])
        # print(batch['data_key'].shape) #torch.Size([128])
        """"
        context 是实际的时间序列数据
        data_key 是这个时间序列属于哪个数据集的标识符
        """
    ...
    def shared_step(self, batch, **kwargs):
        ...
        loss = self(x, c, **kwargs) #调用LatentDiffusion的forward函数
        return loss
    ...
    def apply_model(self, x_noisy, t, cond, mask, cfg_scale=1, cond_drop_prob=None, 
                    sampled_concept= None, sampled_index= None, sub_scale=None,y=None, **kwargs):
        ...
        #调用ts_unet
        if cond_drop_prob is None:#推理
            x_recon = self.model.cfg_forward(x_noisy, t, cfg_scale=cfg_scale, sampled_concept = sampled_concept, sampled_index = sampled_index, sub_scale = sub_scale,y=y, **cond)
        else:#训练
            x_recon = self.model.forward(x_noisy, t, cond_drop_prob=cond_drop_prob, 
                                            sampled_concept = sampled_concept, sampled_index = sampled_index, sub_scale = sub_scale,y=y, **cond)
    ...
    def divide_x_y(self,x_y):
        if self.use_label:
            pass
            data_x = x_y[..., :-4, :]
            labels_y = x_y[..., -4:,:]
            labels_y = labels_y[:, 0]
            return data_x, labels_y.to(torch.long)
        elif self.alternate:
            data_x = x_y[...,:-4, :]
            moe_weights = x_y[..., -4:,:]
            # print(data_x.shape,moe_weights.shape)
            # print('1111')
            return data_x,None
    ...
    def p_losses(self, x_start, condmask, t, noise=None, data_key=None):
        x_start,y=self.divide_x_y(x_start)
        noise = default(noise, lambda: torch.randn_like(x_start))
        ...
        x_noisy = self.q_sample(x_start=x_start, t=t, noise=noise)
        #应用UNet预测噪声
        model_output = self.apply_model(x_noisy, t, cond, mask, cond_drop_prob=self.cond_drop_prob,y=y)

        eps_pred = return_wrap(model_output, extract_into_tensor(self.shift_coef, t, x_start.shape))

        loss_dict = {}
        prefix = 'train' if self.training else 'val'
        # print(self.parameterization)#eps
        # print(noise.shape) #torch.Size([128, 1, 168])
    ...
    def p_mean_variance(self, x, c, t, m, clip_denoised: bool, return_x0=False, y=None,
                        score_corrector=None, corrector_kwargs=None, **kwargs):
        t_in = t
        model_out = self.apply_model(x, t_in, c, m,y=y,**kwargs)
    ...
    def p_sample(self, x, c, t, m, clip_denoised=False, repeat_noise=False,
                 return_x0=False, temperature=1., noise_dropout=0.,y=None,
                 score_corrector=None, corrector_kwargs=None,**kwargs):
        b, *_, device = *x.shape, x.device
        outputs = self.p_mean_variance(x=x, c=c, t=t, m=m, clip_denoised=clip_denoised,y=y,
    ...
    def p_sample_loop(self, cond, shape, return_intermediates=False,
                      x_T=None, verbose=True, callback=None, timesteps=None, quantize_denoised=False,y=None,
        ...
        for i in iterator:
            ...
            seq = self.p_sample(seq, cond, ts, mask,y=y,
                                clip_denoised=self.clip_denoised,
    ...
    def sample(self, cond, batch_size=16, return_intermediates=False, x_T=None,
               verbose=True, timesteps=None, mask=None,y=None, x0=None, shape=None,**kwargs):
        ...
        if cond is not None:
            if isinstance(cond, dict):
                ...
                cond = [c[:batch_size] for c in cond] if isinstance(cond, list) else cond[:batch_size]
        return self.p_sample_loop(cond, shape, return_intermediates=return_intermediates, x_T=x_T,y=y,
                                  verbose=verbose, timesteps=timesteps, mask=mask, x0=x0,**kwargs)

    @torch.no_grad()
    def sample_log(self,cond,batch_size,ddim, ddim_steps=20,y=None,**kwargs):
        ...
        if ddim:
            ...
            samples, intermediates = self.sample(cond=cond, batch_size=batch_size,y=y,
    ...
    #
    #         # Wrap the custom scheduler in a PyTorch-compatible format
    #         from torch.optim.lr_scheduler import _LRScheduler
    #
    #         class SchedulerWrapper(_LRScheduler):
    #             def __init__(self, optimizer, custom_scheduler, last_epoch=-1):
    #                 self.custom_scheduler = custom_scheduler
    #                 super().__init__(optimizer, last_epoch)
    #
    #             def get_lr(self):
    #                 # Call the schedule method of the custom scheduler
    #                 multiplier = self.custom_scheduler.schedule(self._step_count)
    ...
    #                     param_group['lr'] = lr
    #
    #         wrapped_scheduler = SchedulerWrapper(opt, scheduler)
    #
    #         return {
    #             "optimizer": opt,
    #             "lr_scheduler": {
...
class DiffusionWrapper(pl.LightningModule):
    ...
    def forward(self, x, t, c_crossattn: list = None, cond_drop_prob = 0.,y=None, mask=None, **kwargs):

        if (c_crossattn is not None) and (not None in c_crossattn):
            cc = torch.cat(c_crossattn, 1)
        else:
            cc = None
        out = self.diffusion_model(x, t, context=cc, mask=mask,y=y, cond_drop_prob=cond_drop_prob)
        
        return out
        
    def cfg_forward(self, x, t, c_crossattn: list = None, mask=None,y=None, **kwargs):
        
        if (c_crossattn is not None) and (not None in c_crossattn):
            cc = torch.cat(c_crossattn, 1)
        else:
            cc = None
        out = self.diffusion_model.forward_with_cfg(x, t, y=y,context=cc, mask=mask, **kwargs)
